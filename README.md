# Flark - Your Virtual Desktop Buddy 🐾

Flark is a virtual desktop companion designed to help you build healthy work habits. Flark lives as a transparent widget on your screen, keeping an eye on your Pomodoro sessions, making sure you take breaks, and keeping you company with customizable dialogue. If you consistently skip breaks or work too late, Flark's health drops, and he'll start aggressively asking for breaks or even eating your clipboard data!
...
### 1. `main.py`
The entry point of the application. It initializes all the major subsystems (Config, Timer, State Machine, GUI) and wires them up. It also creates the `pystray` system tray icon (which runs in a background thread) so you can easily show or quit Flark without cluttering your taskbar.
...
- **Interactions**: Allows you to click and drag Flark around the screen, or right-click him to open a context menu to manage the timer or make him say things.
...
### 3. `state_machine.py`
The Brain behind Flark's behavior and health. 
- **FlarkHealth**: Tracks Flark's HP (0-100). Taking full breaks heals him (+10), while skipping breaks damages him (-5).
- **StateMachine**: Evaluates the world state based on the current time and the `PomodoroTimer`. Determines if Flark is in `Deep Work`, `Break Time`, `Neglect` (ignored break), `Late Night` (past bedtime), or `Healthy`.
...
- `dialogue.json` stores all the actual strings Flark can express during events (like starting a break), randomly wandering in certain states, or complaining when his health drops during the punishment phases.
...
python main.py
```
Flark will appear in the bottom-right corner of your screen. 
- **Left-Click + Drag:** Move him anywhere.
- **Right-Click:** Pause/Start tracking, skip a phase, make him speak, or hide him.
- **System Tray:** Use the "Flark" icon in your Windows System Tray to bring him back if you hid him!
